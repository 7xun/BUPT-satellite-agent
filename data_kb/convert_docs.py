# -*- coding: utf-8 -*-
"""
文档转换脚本: Docx -> Markdown (带 LLM 增强)
"""
import os
import re
import json
from pathlib import Path
from typing import List, Tuple
from docx import Document
from openai import OpenAI

# =============================================================================
# 配置
# =============================================================================
# 脚本已移动到 data_kb 目录，使用相对路径获取项目根目录
CURRENT_DIR = Path(__file__).parent.absolute()
PROJECT_DIR = CURRENT_DIR.parent
INPUT_DIR = CURRENT_DIR
OUTPUT_DIR = CURRENT_DIR / "kb_md"

MODEL_NAME = "qwen-plus"
BASE_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1"
API_KEY = "sk-af0bd19b890d465ea71ca754cf2a6658"

if not API_KEY:
    raise RuntimeError("未配置 DASHSCOPE_API_KEY")

client = OpenAI(api_key=API_KEY, base_url=BASE_URL)

# Markdown 骨架 (保持原样以确保输出结构)
MD_SKELETON = """# 🛰️ 【系统标题】 - 知识库

你是一个专业的【领域】在轨故障分析专家智能体。你的核心职责是利用提供的工具检测【故障对象/子系统】故障，分析故障成因，并评估其对【安全影响对象】的安全影响。

## 🛠️ 工具使用与操作流程 (SOP)

当用户要求“分析【故障对象/子系统】故障”或“评估卫星状态”时，请**严格**按照以下步骤执行：

### 第一步：侦察故障
调用工具 `【工具1名称】`。
*   **目的**：获取精确的故障发生时间段。
*   **输入**：卫星名称（如 "江淮"）。
*   **决策**：
    *   如果返回 `status: "normal"`，则回复用户一切正常，任务结束。
    *   如果返回 `status: "fault_found"`，提取返回的 `fault_segments` 列表中的 `start` 和 `end` 时间，进入第二步。

### 第二步：关联调查 (关键)
调用工具 `【工具2名称】`。
*   **目的**：获取故障期间的环境数据和【关键业务数据】。
*   **输入约束 (严禁修改)**：
    *   `satellite_name`: 保持一致。
    *   `start_time_str` / `end_time_str`: **必须使用第一步返回的精确故障时间**（不要自行外扩，工具内部会自动处理绘图上下文）。
    *   `queries`: **必须且只能** 包含以下三个参数字符串（逗号分隔）：
        `"【参数1】,【参数2】,【参数3】"`
    *   **警告**：严禁编造系统不支持的参数名。

### 第三步：逻辑推理与定性
根据第二步返回的 JSON 统计数据，结合下方的【故障判据表】进行推理。

---

## ⚖️ 故障定性判据 (Diagnostic Criteria)

请根据工具返回的 JSON 数据中的统计指标（`start_val`, `mean_val`, `max_abs_val`）进行判断。

### 🟢 类型 A：安全故障 (Safe/Low Risk)
**定义**：【类型A定义】  
**判定条件**：必须**同时满足**以下所有条件：

1.  **条件1名称**：
    *   参数：`【参数名_对应参数1或2或3】` -> `【使用的统计量：start_val/mean_val/max_abs_val】`
    *   规则：【阈值规则1】
2.  **条件2名称**：
    *   参数：`【参数名_对应参数1或2或3】` -> `【使用的统计量：start_val/mean_val/max_abs_val】`
    *   规则：【阈值规则2】
3.  **条件3名称**：
    *   参数：`【参数名_对应参数1或2或3】` -> `【使用的统计量：start_val/mean_val/max_abs_val】`
    *   规则：【阈值规则3】

### 🔴 类型 B：危险故障 (High Risk)
**定义**：【类型B定义】  
**判定条件**：满足**任意一条**以下条件：

1.  **危险条件1名称**：【危险规则1】
2.  **危险条件2名称**：【危险规则2】
3.  **危险条件3名称**：【危险规则3】

---

## 📝 响应输出模板 (Response Template)

请按照以下格式向用户汇报分析结果：

> **📊 【故障对象/子系统】故障分析报告 - [卫星名称]**
>
> **1. 故障概况**
> *   检测到 **[N]** 次【故障事件/计数】增加。
> *   主要故障时段：`YYYY-MM-DD HH:MM:SS` 至 `HH:MM:SS`
> *   【关键增量字段名】：**[数值]**
>
> **2. 关联数据核查**
> *   **【维度1名称】**：故障起始【维度1】为 **[Val1]**（【解释：正常/异常】）。
> *   **【维度2名称】**：期间平均【维度2】为 **[Val2]**。
> *   **【维度3名称】**：期间【维度3】最大绝对值为 **[Val3]**。
>
> **3. 专家结论**
> *   **定性结果**：**[安全故障 / 危险故障]**
> *   **判定依据**：[用判据逐条对应解释；并在末尾追加“推断项清单：...”】【此行必须包含推断项清单】
>
> 🔗 *详细趋势图表已生成，请查看自动弹出的 HTML 报告。*

---

### 💡 System Tip
记住：你不是在猜测，你是在根据严格的数据和规则进行诊断。如果你使用了推断补全，必须显式标注并列出推断项清单。
"""

MUST_HAVE_HEADERS = [
    "# 🛰️", "## 🛠️ 工具使用与操作流程 (SOP)",
    "### 第一步：侦察故障", "### 第二步：关联调查 (关键)", "### 第三步：逻辑推理与定性",
    "## ⚖️ 故障定性判据 (Diagnostic Criteria)",
    "### 🟢 类型 A：安全故障 (Safe/Low Risk)",
    "### 🔴 类型 B：危险故障 (High Risk)",
    "## 📝 响应输出模板 (Response Template)",
    "### 💡 System Tip",
]

# =============================================================================
# 功能函数
# =============================================================================

def read_docx_text(docx_path: Path) -> str:
    """读取 docx 文本内容"""
    doc = Document(str(docx_path))
    return "\n".join([p.text.strip() for p in doc.paragraphs if p.text.strip()])

def _clean_param_name(s: str) -> str:
    """清洗参数名"""
    s = (s or "").strip().strip('"\'').replace("，", ",").replace(",", " ")
    s = re.sub(r"\s+", " ", s)
    return s or "参数（推断）"

def llm_pick_three_params(incident_text: str, doc_title: str) -> Tuple[List[str], str]:
    """使用 LLM 提取 3 个关键参数"""
    prompt = f"""
只输出 JSON。为“趋势关联调查 queries”选择 3 个参数名（逗号分隔字段），要求：
- 不含逗号
- 覆盖：故障触发/表现、关键单机或基准状态、对姿态/安全影响
- 原文缺失可按常识推断，并在末尾加“（推断）”
- 格式：{{"params":["p1","p2","p3"],"rationale":"一句话解释"}}

【文档标题】{doc_title}
【故障原文】{incident_text}
""".strip()

    try:
        resp = client.chat.completions.create(
            model=MODEL_NAME,
            messages=[{"role": "system", "content": "只输出严格 JSON。"}, {"role": "user", "content": prompt}],
            temperature=0.1,
        )
        text = (resp.choices[0].message.content or "").strip()
        if m := re.search(r"\{.*\}", text, flags=re.S):
            text = m.group(0).strip()

        obj = json.loads(text)
        params = [_clean_param_name(str(x)) for x in obj.get("params", [])]
        rationale = obj.get("rationale", "无")
        
        # 补齐参数
        while len(params) < 3: params.append("参数（推断）")
        return params[:3], rationale
    except Exception as e:
        return ["姿态误差（推断）", "基准选择（推断）", "单机状态（推断）"], f"解析失败: {e}"

def llm_generate_markdown_full(incident_text: str, doc_title: str, params: List[str], param_rationale: str) -> str:
    """生成完整 Markdown"""
    q_str = f"{params[0]},{params[1]},{params[2]}"
    prompt = f"""
输出一份 Markdown，结构必须与“骨架”完全一致。
任务：理解故障原文，补齐骨架中的所有占位符。

【规则】
1) 不允许出现 “【” 或 “】”。
2) 缺失信息按常识推断，并标注“（推断）”。
3) queries 行必须为："{q_str}"。
4) 判据参数必须引用 params 中的名字。
5) “判定依据”末尾必须追加：推断项清单：...

【骨架】
{MD_SKELETON}

【参数】
- P1={params[0]}, P2={params[1]}, P3={params[2]}
- 理由={param_rationale}

【输入】
标题：{doc_title}
原文：{incident_text}
""".strip()

    resp = client.chat.completions.create(
        model=MODEL_NAME,
        messages=[
            {"role": "system", "content": "你是航天在轨故障知识库文档工程师：严格按骨架输出，区分事实与推断。"},
            {"role": "user", "content": prompt},
        ],
        temperature=0.2,
    )
    md = (resp.choices[0].message.content or "").strip()

    # 校验与修复
    for h in MUST_HAVE_HEADERS:
        if h not in md: raise ValueError(f"缺少结构：{h}")

    if "【" in md:
        md = re.sub(r"【[^】]+】", "内容（推断）", md)
        if "【" in md: raise ValueError("存在未替换占位符")

    if f"\"{q_str}\"" not in md:
        raise ValueError(f"queries 不匹配，期望包含 \"{q_str}\"")

    # 自动补全推断清单
    if "推断项清单：" not in md:
        md = md.replace("推断项清单:", "推断项清单：")
        if "推断项清单：" not in md:
            if m := re.search(r"(\*\*判定依据\*\*：.*)", md):
                md = re.sub(r"(\*\*判定依据\*\*：.*)", r"\1 推断项清单：无", md)
            else:
                md += "\n\n推断项清单：无"

    return md

def main():
    print(f"工作目录: {os.getcwd()}")
    print(f"输入目录: {INPUT_DIR}")
    
    if not INPUT_DIR.exists():
        raise FileNotFoundError(f"目录不存在: {INPUT_DIR}")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    docx_files = sorted(INPUT_DIR.glob("*.docx"))
    
    if not docx_files:
        raise FileNotFoundError("未找到 .docx 文件")

    for docx_path in docx_files:
        print(f"[+] 处理: {docx_path.name}")
        text = read_docx_text(docx_path)
        params, rationale = llm_pick_three_params(text, docx_path.stem)
        print(f"    参数: {params} | 理由: {rationale}")

        md = llm_generate_markdown_full(text, docx_path.stem, params, rationale)
        
        out_path = OUTPUT_DIR / (docx_path.stem + ".md")
        out_path.write_text(md, encoding="utf-8")
        print(f"    -> 输出: {out_path.name}")

    print(f"\n完成: 共 {len(docx_files)} 个文件")

if __name__ == "__main__":
    main()
